...
# --- Core Helper Functions (Unchanged from your script) ---
...
# --- Main Execution Logic (Modified for Folder Processing) ---

def main():
    parser = argparse.ArgumentParser(
        description="Finds all .pcap.xz files in a target directory, converts them, and combines them into a single Parquet file. "
                    "By default, it targets the 'MalScape-1/parquet/Combine/' directory relative to its own location."
    )
    # The 'input_folder' argument is now optional.
    parser.add_argument(
        "input_folder", 
        nargs='?', 
        default=None,
    ...
    # --- 1. Determine target directory and create it if it doesn't exist ---
    if args.input_folder:
        # User has provided a specific folder to use.
        target_folder = args.input_folder
    else:
        # Default behavior: Find the ../parquet/Combine directory relative to this script.
        # This script is in .../MalScape-1/tools/
        script_path = os.path.abspath(__file__)
        tools_dir = os.path.dirname(script_path)
        # Go up one level to get the project root (.../MalScape-1/)
    ...
    if args.output_file:
        ...
        # Default output file name inside the target folder
        output_file = os.path.join(target_folder, "combined_output.parquet")
    ...
    try:
        ...
        files_to_process.sort() # Process in a consistent order
    except FileNotFoundError:
    ...
    # --- 4. Loop through files, process them, and collect DataFrames ---
    all_dataframes = []
    for filename in files_to_process:
        ...
        try:
            ...
            # Clean up the temporary decompressed .pcap file
    ...
    # --- 5. Combine, sort, and save the final DataFrame ---
    ...
    if 'Time' in combined_df.columns and pd.api.types.is_datetime64_any_dtype(combined_df['Time']):
    ...
    print(f"Saving combined DataFrame with {len(combined_df)} rows to '{output_file}'...")
    combined_df.to_parquet(output_file, index=False, engine='pyarrow')

    print("\n--- Conversion Complete! ---")
    print(f"Successfully processed {len(all_dataframes)} files and created '{output_file}'.")
